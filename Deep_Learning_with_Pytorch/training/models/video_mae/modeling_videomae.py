...
#
# ------------------------------------------------------------------------------
# Copied from https://github.com/huggingface/transformers/tree/main/src/transformers/models/vit_mae (2024/3/14 main)
# パディングフレームに対するマスクとFlashAttentionを追加した
...
class VideoMAEPatchEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values):
        _, _, num_channels, height, width = pixel_values.shape
...
class VideoMAESelfAttention(nn.Module):
    def __init__(
        self, config: VideoMAEConfig, use_memory_efficient_attention=False
    ) -> None:
        super().__init__()
    ...
    def transpose_for_scores(self, x: torch.Tensor, permute=True) -> torch.Tensor:
    ...
    def _apply_attention_mask(
        self, attention_scores: torch.Tensor, attention_mask: torch.BoolTensor
    ) -> torch.Tensor:
        """Args:
        attention_scores: torch.Tensor of shape (batch_size, num_heads, seq_len, seq_len)
        attention_mask: torch.Tensor of shape (batch_size, seq_len)
                        1 for masked positions, 0 for visible positions
        """
        ...
        attention_scores = attention_scores * (~attention_mask).to(
            dtype
        ) + bias_value * attention_mask.to(dtype)
    ...
    def forward(
        ...
        orginal_shape = hidden_states.shape
        ...
        if not self.use_memory_efficient_attention:
            ...
            attention_probs = nn.functional.softmax(
                attention_scores, dim=-1, dtype=values.dtype
            )
            ...
            context_layer = context_layer.view(orginal_shape)
        else:
            # memory efficient attention
            keys = self.transpose_for_scores(keys, permute=False)
            values = self.transpose_for_scores(values, permute=False)
            queries = self.transpose_for_scores(queries, permute=False)
            ...
            )  # shape(batch_size=1, accumulated_seq_len, num_head, head_dim)
            context_layer = context_layer.view(
                orginal_shape
            )  # shape(batch_size=1, accumulated_seq_len, all_head_size)
...
class VideoMAEAttention(nn.Module):
    def __init__(
        self, config: VideoMAEConfig, use_memory_efficient_attention: bool = False
    ) -> None:
        super().__init__()
        self.attention = VideoMAESelfAttention(
...
class VideoMAELayer(nn.Module):
    ...
    def __init__(
        ...
        self.seq_len_dim = 1
        self.attention = VideoMAEAttention(
            config, use_memory_efficient_attention=use_memory_efficient_attention
        )
...
class VideoMAEEncoder(nn.Module):
    ...
    def _get_attention_mask(self, frame_mask: torch.BoolTensor) -> torch.BoolTensor:
        """
        Get patch attention mask from a frame mask tensor.
        Args:
            frame_mask: torch.BoolTensor of shape (batch_size, num_frames)
                        1 for masked frames, 0 for visible frames
        Returns:
            torch.Tensor of shape (batch_size, seq_len)
                        1 for masked positions, 0 for visible positions
        """
        # 時間方向のパッチサイズを考慮して、frame_maskを縮小する
        _frame_mask = frame_mask[:, :: self.config.tubelet_size]
        # フレーム1枚分のパッチ数
        image_size = (
        ...
        patch_size = (
            ...
            if isinstance(self.config.patch_size, collections.abc.Iterable)
            else (self.config.patch_size, self.config.patch_size)
        )
        num_patches_per_frame = (image_size[0] // patch_size[0]) * (
            image_size[1] // patch_size[1]
        )
        # _frame_maskをパッチ数分繰り返す
        attention_mask = _frame_mask.repeat_interleave(num_patches_per_frame, dim=1)
        return attention_mask

    def _cumulate_sequence(self, x, attention_mask):
        """Remove padding and cumulate x to the sequence dimension
        Args:
            x (torch.Tensor): shape(batch_size, seq_len, ...)
            attention_mask (torch.BoolTensor): shape(batch_size, seq_len), True for padded positions
        Returns:
            x (torch.Tensor) : shape(1, cumulated sequence, ...)
            attn_bias (xformers.ops.fmha.attn_bias.BlockDiagonalMask)
        """
        x_list = [
            xub[~mask].unsqueeze(0) for xub, mask in zip(x.unbind(0), attention_mask)
        ]
        attn_bias, x = BlockDiagonalMask.from_tensor_list(x_list)
        return x, attn_bias

    def _uncumulate_sequence(self, x, attn_bias, max_seqlen=None):
        """
        Args:
            x (torch.Tensor): shape(1, cumulated sequence, ...)
            attn_bias (xformers.ops.fmha.attn_bias.BlockDiagonalMask):
            max_seqlen (int, optional): Sequence is paded to this length.
                If not given, attn_bias.q_seqinfo.max_seqlen is used.
        Returns:
            x (torch.Tensor): shape(batch_size, seq_len, ...)
            pad_pos (torch.BoolTensor): shape(batch_size, seq_len), True for padded positions
        """
        list_x = attn_bias.split(x)

        max_seqlen = (
            attn_bias.q_seqinfo.max_seqlen if max_seqlen is None else max_seqlen
        )
        x, pad_pos = self._pad_sequences(list_x, max_seqlen)
        return x, pad_pos

    def _pad_sequences(self, list_x, max_seqlen):
        seqlen_list = [x.size(1) for x in list_x]
        d = len(list_x[0].shape)
        pads = [(0, 0) * (d - 2) + (0, max_seqlen - seqlen) for seqlen in seqlen_list]
        padded_tensors = [nn.functional.pad(x, pad) for x, pad in zip(list_x, pads)]
        pad_pos_list = [
            torch.cat([torch.zeros(seqlen), torch.ones(max_seqlen - seqlen)])
            for seqlen in seqlen_list
        ]
        return torch.cat(padded_tensors, dim=0), torch.stack(pad_pos_list)

    def forward(
        self,
        hidden_states: torch.Tensor,
        frame_mask: Optional[torch.BoolTensor] = None,
        head_mask: Optional[torch.Tensor] = None,
        output_attentions: bool = False,
        output_hidden_states: bool = False,
    ) -> Union[tuple, ModelOutput]:
        org_seq_len = hidden_states.shape[1]
        ...
        if frame_mask is not None:
            attention_mask = self._get_attention_mask(frame_mask)
        ...
        if self.use_memory_efficient_attention:
            ...
            hidden_states, attn_bias = self._cumulate_sequence(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if output_hidden_states:
                if self.use_memory_efficient_attention:
                    hs, _ = self._uncumulate_sequence(
                        hidden_states, attn_bias, max_seqlen=org_seq_len
                    )
                else:
                    hs = hidden_states
        ...
        if self.use_memory_efficient_attention:
            hidden_states, _ = self._uncumulate_sequence(
                hidden_states, attn_bias, max_seqlen=org_seq_len
            )
...
class VideoMAEModel(VideoMAEPreTrainedModel):
    def __init__(self, config, use_memory_efficient_attention=False):
    ...
    def forward(
        ...
        frame_mask (`torch.BoolTensor` of shape `(batch_size, num_frames)`, *optional*):
            Boolean mask for each frame in the video. If `None`, then all frames are considered.
            Indicates which frames are masked (1) and which aren't (0).
        ...
        [1, 1568, 768]
        ```"""
        # frame_maskとbool_masked_posを同時に使うことはサポート外
        if frame_mask is not None and bool_masked_pos is not None:
            raise ValueError(
                "frame_mask and bool_masked_pos cannot be used at the same time."
            )
        # memory_efficient_attention使用時はoutput_attentionsは無効
        if self.use_memory_efficient_attention:
